...
globals [
  ...
  drains          ;; agentset of all edge patches where the brainmap1 is
  land            ;; agentset of all non-edge patches
]

patches-own [
  elevation       ;; elevation here (may be negative)
  brainmap1           ;; depth of brainmap1 here
...
to setup
  clear-all
  fetch:url-async "https://NetLogo-BrainBuds.github.io/pixlrbrain.png" import-a:pcolors
  set show-brainmap1? true
  ask patches [
    set brainmap1 pcolor
  ]


  create-turtles marijuana_amt [
    set shape "leaf"
    set color yellow ;orbitofrontal cortex
    setxy random-xcor random-ycor
    set size 4
  ]
  create-turtles marijuana_amt [
    set shape "leaf"
    set color red ;hippocampal gyrus
    setxy random-xcor random-ycor
    set size 4
  ]
  create-turtles marijuana_amt [
    set shape "leaf"
    set color pink ;temporal pole
    setxy random-xcor random-ycor
    set size 4
  ]
  create-turtles marijuana_amt [
    set shape "leaf"
    set color blue ;medial temporal cortex
    setxy random-xcor random-ycor
    set size 4
  ]
  create-turtles marijuana_amt [
    set shape "leaf"
    set color green ;insula
    setxy random-xcor random-ycor
    set size 4
  ]
...
to recolor  ;; patch procedure
  ifelse brainmap1 = 0 or not show-brainmap1?
    [ set pcolor scale-color white elevation -250 100 ]
    [ set pcolor scale-color blue (min list brainmap1 75) 100 -10 ]
end
...
to go
  ask turtles [
    ...
    [
     ...
     if color = pink
    [
      set heading towardsxy -19 -20
      if [pcolor] of patch-here = 125.7
      [
        ;ask patches
        ;[
            set pcolor black
     ...
     if color = green
    [
      set heading towardsxy -13 6
      if [pcolor] of patch-here = 65.5
      [
        ;ask patches
        ;[
            set pcolor black
     ...
     if color = blue
    [
      set heading towardsxy -7 -17
      if [pcolor] of patch-here = 104.8
      [
        ;ask patches
        ;[
            set pcolor black
...
This model is a simulation of the brain on various substances.  The user is presented with an empty brain.  Substances represented by leaves move around the brain space.  As the substances move around the brain, they are attracted to areas with CB1 receptors.
